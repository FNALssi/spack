...
class BinaryTextReplaceException(Exception):
    """
    Raised when the new install path is shorter than the old install path
    so binary text replacement cannot occur unless allow_root is set.
    """

    def __init__(self, old_path, new_path):
        err_msg = "The old path %s" % old_path
        err_msg += " is shorter than new path %s.\n" % new_path
        err_msg += "Text replacement in binaries will not work.\n"
        super(BinaryTextReplaceException, self).__init__(err_msg)
...
def relocate_package(spec, workdir, allow_root):
    ...
    new_prefix = spec.prefix
    old_path = buildinfo['buildpath']
    old_spack_prefix = buildinfo.get('spackprefix',
                                     '/not/in/buildinfo/dictionary')
    old_rel_prefix = buildinfo.get('relative_prefix',
                                   'not/in/buildinfo/dictionary')
    old_prefix = old_path + os.sep + old_rel_prefix
    new_spack_prefix = spack.paths.prefix
    rel = buildinfo.get('relative_rpaths', False)

    tty.debug("Relocating package from install root ",
              "%s to %s." % (old_path, new_path))
    tty.debug("Relocating package from install path ",
              "%s to %s." % (old_prefix, new_prefix))
    tty.debug("Relocating package from spack path",
              "%s to %s." % (old_spack_prefix, new_spack_prefix))
    path_names = set()
    path_names = set()
    ...
    relocate.relocate_text(path_names,
                           ...
                           oldsprefix=old_spack_prefix,
                           newsprefix=new_spack_prefix)
